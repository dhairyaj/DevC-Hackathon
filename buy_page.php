...
'.$_SESSION['name'].' has shown interest to buy your product. 

-------------------------------
Product Name: '.$row['iname'].'
Item id: '.$row['iid'].'    
Price Offering: '.$_POST['oprice'].'
...
<body id="top">
                    ...
                    <div class="header-nav">
                        ...
                        <form method="post">
                            <ul class="primary-nav">
                                ...
                                <li><a><span class="active">Buy</span></a></li>
    ...
    <div class="jumbotron" style="margin: 0; padding-left: 10px; padding-top: 10px; min-height: 500px;">
        ...
        <div id="right_bar">    
            ...
            <select autocomplete=off name="itype" id="i_type" style="font-size: 120%; margin-bottom: 20px;">
        ...
        <?php
            if(!isset($_POST['go'])) {
                $query = "SELECT * FROM `items` WHERE `sell` = 1 AND `status` = 0 ORDER BY `id` DESC";
            ...
            if(isset($_POST['go'])) {
                if(!empty($_POST['itype']) AND $_POST['itype']!=1) {
                    ...
                    if($select != "" && $_POST[$select]!=1) {
                        $found=1;
                        $query = "SELECT * FROM `items` WHERE itype='".mysqli_real_escape_string($link, $typeList[$type])."' AND isubtype='".mysqli_real_escape_string($link, $_POST[$select])."' AND `sell` = 1 AND `status` = 0";
                        $result = mysqli_query($link, $query);
                        ...
                        $query = "SELECT * FROM `items` WHERE `sell` = 1 AND `status` = 0";
                        $result = mysqli_query($link, $query);
                        echo "<div class='row'>";
                        while($row = mysqli_fetch_assoc($result)) {
                            $iid = $row['iid'];
